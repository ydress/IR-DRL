...
class Env(gym.Env):
    ...
    def _reward(self):
        ...
        #Huber-Loss+
        if abs(self.distance) < sigma:
            RE_T = 0.5 * (self.distance ** 2)
        else:
            RE_T = sigma * (abs(self.distance) - 0.5 * sigma)
            
        # distance between the robot end and the obstacle
        #distance_end_obstacle = np.linalg.norm(np.asarray(list(self.current_pos))-np.asarray(self.obstacle_position), ord=None)
        R_0 = 0 #(d_ref / (distance_end_obstacle + d_ref)) ** k
        
        # motion size
        R_A = - np.linalg.norm(self.action)
        
        
        reward = lambda_1 * RE_T #- lambda_2 * R_0 + lambda_3 * R_A
        #print("Reward:", reward)
...
if __name__ == '__main__':
    ...
    env = Env(is_render=True, is_good_view=False,add_moving_obstacle=False)
    episodes = 100
