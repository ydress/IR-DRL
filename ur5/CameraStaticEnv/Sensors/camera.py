...
class Camera():
    ...
    def get_camera_data(self, cameraType="RGB"):
        width = 64
        height = 64
        ...
        rgb_tiny = np.transpose(np.reshape(images[2], (height, width, 4)),  (2, 0, 1))
        #print(rgb_tiny.shape)
        
        if cameraType == "RGB":
           #print(rgb_tiny.shape)
            #print(rgb_tiny.min(), rgb_tiny.mean())
            return rgb_tiny
