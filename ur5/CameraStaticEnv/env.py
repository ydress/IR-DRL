...
class Env(gym.Env):
    ...
    def _add_obstacles(self):
        ...
        for item in range(3):
            if np.random.random()>0.3:
                i = choice([0,1,2])
                obstacle_position = 0.5*(np.array(self.init_home)+np.array(self.target_position))+0.05*np.random.uniform(low=-1, high=1, size=(3,))
                if i==0:
    ...
    def reset(self):
        ...
        self.rgb = self.camera1.get_camera_data()
    ...
    def step(self,action):
        ...
        # update current pose
        self. nb  = p.getLinkState(self.RobotUid,self.effector_link)[4]
        self.current_orn = p.getLinkState(self.RobotUid,self.effector_link)[5]
        self.wrist3_pos = p.getLinkState(self.RobotUid,6)[4]
        self.wrist3_orn = p.getLinkState(self.RobotUid,6)[5]
        self.wrist2_pos = p.getLinkState(self.RobotUid,5)[4]
