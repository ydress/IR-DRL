...
def make_env(rank: int, seed: int = 42) -> Callable:
    """
...
if __name__ == '__main__':
    ...
    action_noise = NormalActionNoise(mean=np.zeros(n_actions), sigma=np.ones(n_actions))

    # Create the callback list
    callback = CallbackList([checkpoint_callback, callback_max_episodes, eval_callback])
    model = TD3("MultiInputPolicy", env, train_freq=1, learning_rate=0.001, tau=0.001, gamma=0.99,
                action_noise=action_noise, verbose=1, tensorboard_log=f'./models/{run_name}/tf_logs/')
    #model = PPO("MultiInputPolicy", env, learning_rate=0.001, verbose=1, tensorboard_log=f'./models/{run_name}/tf_logs/')
    #model = PPO.load('./models/RUN_6PPO/reach_3M.zip', env=env)
    model.learn(
        total_timesteps=3000000,
        n_eval_episodes=64,
        callback=callback)
    model.save(f'./models/{run_name}/reach_3M')
